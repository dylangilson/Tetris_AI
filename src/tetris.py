...
class Tetris:
    ...
    def reset(self):
        self.board = [[self.MAP_EMPTY] * self.BOARD_WIDTH for _ in range(self.BOARD_HEIGHT)]
        self.board_colours = [[self.MAP_EMPTY] * self.BOARD_WIDTH for _ in range(self.BOARD_HEIGHT)]
    ...
    def new_round(self, piece_fall=False):
        ...
        if piece_fall:
            ...
            self.score += 1 + (lines_cleared ** 2) * self.BOARD_WIDTH
    ...
    def is_valid_position(self, piece, pos):
        for x, y in piece:
            ...
            if x < 0 or x >= self.BOARD_WIDTH or y < 0 or y >= self.BOARD_HEIGHT or self.board[y][x] == self.MAP_BLOCK:
                return False
    ...
    def clear_rows(self, board):
        lines_to_clear = [index for index, row in enumerate(board) if sum(row) == self.BOARD_WIDTH]

        if lines_to_clear:
            board = [row for index, row in enumerate(board) if index not in lines_to_clear]

            for _ in lines_to_clear:
                board.insert(0, [self.MAP_EMPTY for _ in range(self.BOARD_WIDTH)])
    ...
    def number_of_holes(self, board):
        ...
        for column in zip(*board):
            ...
            while i < self.BOARD_HEIGHT and column[i] != self.MAP_BLOCK:
                i += 1

            holes += len([x for x in column[i + 1:] if x == self.MAP_EMPTY])
    ...
    def height(self, board):
        ...
        for column in zip(*board):
            ...
            while i < self.BOARD_HEIGHT and column[i] == self.MAP_EMPTY:
                i += 1
    ...
    def hard_drop(self, position, rotation, render=False):
        ...
        self.board = self.add_piece_to_board(piece, self.current_position)
        lines_cleared, self.board = self.clear_rows(self.board)
        self.score += (lines_cleared ** 2) * self.BOARD_WIDTH
    ...
    def render(self, wait_key=False):
        ...
        self.update_board()
        board = [self.COLOURS[p] for row in self.board_colours for p in row]
        board = self.prepare_image(board, self.BOARD_HEIGHT, self.BOARD_WIDTH)
        ...
        self.get_next_piece()
        next_piece_board = [self.COLOURS[p] for row in self.next_piece_board_colours for p in row]
